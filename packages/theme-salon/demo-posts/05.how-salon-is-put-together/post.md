---
title: How Salon is put together
date: 2026-07-10
template: post
published: true
summary: Templates, the featured lead, and CSS variables worth knowing.
taxonomy:
  tag: [customization, salon]
cover: /themes/salon/static/demo/cover-put-together.jpg
---

Everything so far has been about `config_schema`. This post is for the
rest: what Salon's templates actually do, and a couple of things you can
...
- **`blog`** — featured lead post plus a card grid. Used for home (with
...
Posts can set `cover` for the featured hero and card thumbnails, plus
`taxonomy.tag` (or `tags`) for the pill badges. The first post in the
collection becomes the large featured lead; the rest fill the grid.

This demo vendors a few Unsplash photos under
`themes/salon/static/demo/` (downloaded, not hotlinked) with photographer
credit lines at the foot of each post — see that folder's `CREDITS.md`.
