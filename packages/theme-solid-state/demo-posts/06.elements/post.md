...
template: post
published: true
summary: Headings, lists, quotes, and code in Solid State’s post chrome.
taxonomy:
...
Solid State’s landing chrome (banner + features) is louder than post bodies. This Elements leaf lives under `/blog` so CTAs stay real — it checks that inner articles still get a readable type scale.
