...
template: post
published: true
summary: Theme-specific fidelity notes for Directive — not a generic clone.
taxonomy:
  tag: [chrome, directive, fidelity]
---

Upstream Directive is known for **corporate banner**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Directive-style banner and feature rows.
