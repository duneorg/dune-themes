...
template: post
published: true
summary: Theme-specific fidelity notes for Tessellate — not a generic clone.
taxonomy:
  tag: [chrome, tessellate, fidelity]
---

Upstream Tessellate is known for **tessellated features**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Tessellated feature grid.
