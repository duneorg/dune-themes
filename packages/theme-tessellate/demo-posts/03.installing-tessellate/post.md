...
Tessellate is a scroll-section landing — dark fullscreen header, icon feature
band, image gallery, and closing CTA. You want that shell running against real
Dune content, not a static HTML5 UP unzip sitting next to it.

## From JSR (recommended)

```bash
dune theme:install jsr:@dune/theme-tessellate@1.0.1 --activate
```
...
webfonts ship with the package — no hand-copying files into `themes/tessellate/`.

## From a ZIP

Grab `tessellate-1.0.1.zip` from
[dune-themes releases](https://github.com/duneorg/dune-themes/releases), extract into
`themes/tessellate/`, then set:
