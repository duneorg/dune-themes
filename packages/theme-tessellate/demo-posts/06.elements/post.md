...
template: post
published: true
summary: Headings, lists, quotes, and code in Tessellate’s post chrome.
taxonomy:
...
Tessellate’s landing chrome (tessellated features) is louder than post bodies. This Elements leaf lives under `/blog` so CTAs stay real — it checks that inner articles still get a readable type scale.
