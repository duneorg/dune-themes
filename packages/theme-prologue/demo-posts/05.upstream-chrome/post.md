...
template: post
published: true
summary: Theme-specific fidelity notes for Prologue — not a generic clone.
taxonomy:
  tag: [chrome, prologue, fidelity]
---

Upstream Prologue is known for **avatar sidebar**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Avatar sidebar + scrolly one-page sections.
