...
template: post
published: true
summary: Headings, lists, quotes, and code in Prologue’s post chrome.
taxonomy:
...
Prologue’s landing chrome (avatar sidebar) is louder than post bodies. This Elements leaf lives under `/blog` so CTAs stay real — it checks that inner articles still get a readable type scale.
