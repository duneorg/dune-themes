...
| --- | --- | --- | --- |
| `color_scheme` | select | `blue` | Accent color + sidebar tint. One of `blue`, `slate`, `green`, `purple`, `amber`, `rose`, `terracotta`, `teal` (labels: Cobalt, Emerald, Indigo, Crimson, …) — see [Customization](../customization/) for what each pair actually contains. |
...
| `flat_nav` | toggle | `false` | The sidebar shows the full nested page tree by default (current page's ancestor sections start expanded). Turn this on to show only top-level pages instead. |
| `nav_expand` | select | `auto` | `auto`: sections open only around the current page, no chevron. `click`: adds a chevron so visitors can also expand/collapse sections manually. Ignored when `flat_nav` is on. |
