...
template: post
published: true
summary: Theme-specific fidelity notes for Highlights — not a generic clone.
taxonomy:
  tag: [chrome, highlights, fidelity]
---

Upstream Highlights is known for **highlight bands**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Highlight bands and icon rows.
