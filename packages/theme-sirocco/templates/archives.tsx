...
export default function ArchivesTemplate(props: any) {
  ...
  const monthName = (y: number, m: number) => new Intl.DateTimeFormat(lang, { month: "long" }).format(new Date(y, m, 1));
    ...
    <LayoutComponent {...props}>
      <article class="post-single">
        <header class="post-header">
          ...
          {fm.metadata?.description && <div class="post-description">{fm.metadata.description}</div>}
        </header>
                        ...
                        {posts.map((post: any) => (
                          <li key={post.route}>
                            ...
                            <time datetime={new Date(post.frontmatter.date).toISOString()}>
                              {formatDate(new Date(post.frontmatter.date).getTime(), lang, { day: "numeric", month: "short" })}
                            </time>
                          </li>
                        ))}
                      </ul>
