...
template: post
published: true
summary: Theme options for the Strata demo — still a single upstream design.
taxonomy:
  tag: [config, strata]
---
...
| Option | Notes |
|--------|--------|
| `avatar` | see `theme.yaml` |
| `tagline` | see `theme.yaml` |
| `show_html5up_credit` | see `theme.yaml` |
| `footer_text` | see `theme.yaml` |

**No dark mode / no color-scheme presets.** Upstream Strata is one design;
