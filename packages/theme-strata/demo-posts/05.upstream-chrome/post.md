...
template: post
published: true
summary: Theme-specific fidelity notes for Strata — not a generic clone.
taxonomy:
  tag: [chrome, strata, fidelity]
---

Upstream Strata is known for **avatar + thumbs**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Avatar header + thumbnail portfolio grid.
