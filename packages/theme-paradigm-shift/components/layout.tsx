    ...
    <html lang={page?.language ?? "en"} dir={dir ?? "ltr"} class={defaultDark ? "dark" : ""}>
      <head>
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){var s=localStorage.getItem('theme-paradigm-shift');
      ...
      <body class="theme-paradigm-shift archetype-landing">
        <header class="site-header">
          <a class="site-logo" href="/">{site?.title ?? "Paradigm Shift"}</a>
        ...
        {currentPath === "/" && (
          <section class="hero">
            <h1>{site?.title ?? "Paradigm Shift"}</h1>
        ...
        <footer class="site-footer">
          {footerText || (
            <>
              © {new Date().getFullYear()} {site?.title ?? "Paradigm Shift"} ·{" "}
              {(themeConfig?.show_html5up_credit !== false) && (
                <>
                  <a href="https://html5up.net/paradigm-shift">Paradigm Shift by HTML5 UP</a> ·{" "}
                </>
              )}
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){
            ...
            btn.addEventListener('click',function(){
              ...
              localStorage.setItem('theme-paradigm-shift',dark?'dark':'light');
            });
          })();
        ` }} />
      </body>
