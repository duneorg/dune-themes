---
title: Massively chrome on Dune
date: 2026-03-15
template: post
published: true
summary: Theme-specific fidelity notes for Massively — not a generic clone.
taxonomy:
  tag: [chrome, massively, fidelity]
---

Upstream Massively is known for **hero + cards**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Hero intro and post cards — Elements is the typography check inside a card world.
