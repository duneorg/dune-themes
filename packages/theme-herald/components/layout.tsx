...
export default function Layout(
  ...
  const heroImage = safeHref(themeConfig?.hero_image) ?? "";
      ...
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `
          (function(){
            var s=localStorage.getItem('theme-herald');
      ...
      <body class="theme-herald archetype-blog-hero">
        <header class="site-header herald-header">
          <div class="herald-header-inner">
            <a class="site-logo herald-logo" href={homeHref}>{site?.title ?? "Herald"}</a>
            <button
              type="button"
              class="nav-toggle"
              aria-expanded="false"
            ...
            <div class="herald-header-controls">
              {showSchemeSwitcher && (
        ...
        {showHero && (
          <section class="herald-hero">
            ...
            <div class="herald-hero-content">
              <h1>
                {(themeConfig?.hero_title as string) || site?.title || "Herald"}
              </h1>
              <p>
                {(themeConfig?.hero_subtitle as string) ||
                  site?.description ||
                  tr("hero.subtitle", "Thoughts, stories and ideas.")}
              </p>
            </div>
          </section>
        )}
        {children}
        <footer class="site-footer herald-footer">
          {footerText || (
            <>
              © {new Date().getFullYear()} {site?.title ?? "Herald"} · Powered by{" "}
        ...
        <script
          ...
          (function(){
            ...
            var STORAGE_KEY='herald-color-scheme';
            ...
            if(btn)btn.addEventListener('click',function(){
              ...
              localStorage.setItem('theme-herald',next);
