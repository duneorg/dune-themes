---
title: Configuring Lens
date: 2026-03-08
template: post
published: true
summary: Theme options for the Lens demo — still a single upstream design.
taxonomy:
  tag: [config, lens]
---

| Option | Notes |
|--------|--------|
| `home_subtitle` | see `theme.yaml` |
| `show_html5up_credit` | see `theme.yaml` |
| `footer_text` | see `theme.yaml` |

**No dark mode / no color-scheme presets.** Upstream Lens is one design;
a Dune lift may land later as an optional enhancement, not as fidelity work.
Keep `show_html5up_credit` on unless you hold a separate Pixelarity license.
