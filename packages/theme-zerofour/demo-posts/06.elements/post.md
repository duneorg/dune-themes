...
template: post
published: true
summary: Headings, lists, quotes, and code in Zerofour’s post chrome.
taxonomy:
...
Zerofour’s landing chrome (zerofour boxes) is louder than post bodies. This Elements leaf lives under `/blog` so CTAs stay real — it checks that inner articles still get a readable type scale.
