...
template: post
published: true
summary: Theme-specific fidelity notes for Zerofour — not a generic clone.
taxonomy:
  tag: [chrome, zerofour, fidelity]
---

Upstream Zerofour is known for **zerofour boxes**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Boxy feature landing.
