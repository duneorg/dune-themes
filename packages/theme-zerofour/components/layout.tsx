  ...
  return (
    <html lang={page?.language ?? "en"} dir={dir ?? "ltr"} class="is-preload">
      ...
      <body class="is-preload">
        <header id="header">
          <h1><a href="/">{site?.title ?? "Zerofour"}</a></h1>
        </header>
        <div id="main">{children}</div>
        <footer id="footer">{showCredit && (
          <ul id="copyright">
            <li>&copy; {new Date().getFullYear()} {site?.title ?? "Zerofour"}.</li>
            <li>Design: <a href="https://html5up.net/zerofour">Zerofour by HTML5 UP</a></li>
          </ul>
        )}</footer>
