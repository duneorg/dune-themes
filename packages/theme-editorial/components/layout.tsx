...
export default function Layout({ page, pageTitle, site, config, nav, pathname, dir, children, themeConfig }: LayoutProps) {
  ...
  const accent = (themeConfig?.accent_color as string) ?? "#f2849e";
    ...
    <html lang={page?.language ?? "en"} dir={dir ?? "ltr"} class={defaultDark ? "dark" : ""}>
      <head>
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){var s=localStorage.getItem('theme-editorial');
      ...
      <body class="theme-editorial archetype-blog-magazine">
        <header class="site-header">
          <a class="site-logo" href="/">{site?.title ?? "Editorial"}</a>
        ...
        <footer class="site-footer">
          {footerText || (
            <>
              © {new Date().getFullYear()} {site?.title ?? "Editorial"} ·{" "}
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){
            ...
            btn.addEventListener('click',function(){
              ...
              localStorage.setItem('theme-editorial',dark?'dark':'light');
            });
          })();
        ` }} />
      </body>
