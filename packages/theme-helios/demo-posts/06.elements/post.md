...
template: post
published: true
summary: Headings, lists, quotes, and code in Helios’s post chrome.
taxonomy:
...
Helios’s landing chrome (helios sections) is louder than post bodies. This Elements leaf lives under `/blog` so CTAs stay real — it checks that inner articles still get a readable type scale.
