...
template: post
published: true
summary: Theme-specific fidelity notes for Helios — not a generic clone.
taxonomy:
  tag: [chrome, helios, fidelity]
---

Upstream Helios is known for **helios sections**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Bright sections with carousel-adjacent rhythm.

Compare side-by-side with [html5up.net/helios](https://html5up.net/helios):
