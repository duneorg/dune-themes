---
title: Helios
template: default
published: true
description: >
  Helios adapted from HTML5 UP for Dune — landing banner/sections from upstream, with CTAs pointed at Dune fixtures.
  Single upstream design; no dark mode or color-scheme presets in this port.
---

A faithful-adjacent port of [HTML5 UP Helios](https://html5up.net/helios) for Dune.
Home chrome stays template-driven; Blog / About / Search / Archives exist so
upstream-style CTAs resolve to real Dune routes.
