...
template: post
published: true
summary: Headings, lists, quotes, and code in TXT’s post chrome.
taxonomy:
...
TXT’s landing chrome (text + sidebar) is louder than post bodies. This Elements leaf lives under `/blog` so CTAs stay real — it checks that inner articles still get a readable type scale.
