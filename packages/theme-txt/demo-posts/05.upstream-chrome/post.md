...
template: post
published: true
summary: Theme-specific fidelity notes for TXT — not a generic clone.
taxonomy:
  tag: [chrome, txt, fidelity]
---

Upstream TXT is known for **text + sidebar**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Compact text-forward layout with sidebar nav.
