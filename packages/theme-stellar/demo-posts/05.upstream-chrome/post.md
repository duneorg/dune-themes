...
template: post
published: true
summary: Theme-specific fidelity notes for Stellar — not a generic clone.
taxonomy:
  tag: [chrome, stellar, fidelity]
---

Upstream Stellar is known for **scrolly sections**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Icon features and scrolly sections ending in a footer CTA.
