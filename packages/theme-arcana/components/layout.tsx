    ...
    <html lang={page?.language ?? "en"} dir={dir ?? "ltr"} class={defaultDark ? "dark" : ""}>
      <head>
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){var s=localStorage.getItem('theme-arcana');
      ...
      <body class="theme-arcana archetype-landing">
        <header class="site-header">
          <a class="site-logo" href="/">{site?.title ?? "Arcana"}</a>
        ...
        <footer class="site-footer">
          {footerText || (
            <>
              © {new Date().getFullYear()} {site?.title ?? "Arcana"} ·{" "}
              {(themeConfig?.show_html5up_credit !== false) && (
                <>
                  <a href="https://html5up.net/arcana">Arcana by HTML5 UP</a> ·{" "}
                </>
              )}
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){
            ...
            btn.addEventListener('click',function(){
              ...
              localStorage.setItem('theme-arcana',dark?'dark':'light');
            });
          })();
        ` }} />
      </body>
