    ...
    <html lang={page?.language ?? "en"} dir={dir ?? "ltr"} class={defaultDark ? "dark" : ""}>
      <head>
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){var s=localStorage.getItem('theme-escape-velocity');
      ...
      <body class="theme-escape-velocity archetype-landing">
        <header class="site-header">
          <a class="site-logo" href="/">{site?.title ?? "Escape Velocity"}</a>
        ...
        {currentPath === "/" && (
          <section class="hero">
            <h1>{site?.title ?? "Escape Velocity"}</h1>
        ...
        <footer class="site-footer">
          {footerText || (
            <>
              © {new Date().getFullYear()} {site?.title ?? "Escape Velocity"} ·{" "}
              {(themeConfig?.show_html5up_credit !== false) && (
                <>
                  <a href="https://html5up.net/escape-velocity">Escape Velocity by HTML5 UP</a> ·{" "}
                </>
              )}
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){
            ...
            btn.addEventListener('click',function(){
              ...
              localStorage.setItem('theme-escape-velocity',dark?'dark':'light');
            });
          })();
        ` }} />
      </body>
