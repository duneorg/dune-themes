...
export default function Layout({ page, pageTitle, site, config, nav, pathname, dir, children, themeConfig }: LayoutProps) {
  ...
  const accent = (themeConfig?.accent_color as string) ?? "#0969da";
    ...
    <html lang={page?.language ?? "en"} dir={dir ?? "ltr"} class={defaultDark ? "dark" : ""}>
      <head>
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){var s=localStorage.getItem('theme-syntax');
      ...
      <body class="theme-syntax archetype-blog-tech">
        <header class="site-header syntax-header">
          <div class="syntax-header-inner">
            <a class="site-logo syntax-logo" href="/">{site?.title ?? "Syntax"}</a>
        ...
        <footer class="site-footer">
          {footerText || <>© {new Date().getFullYear()} {site?.title ?? "Syntax"} · Powered by <a href="https://getdune.org">Dune</a></>}
        </footer>
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){
            ...
            btn.addEventListener('click',function(){
              ...
              localStorage.setItem('theme-syntax',dark?'dark':'light');
            });
          })();
        ` }} />
      </body>
