...
template: post
published: true
summary: Headings, lists, quotes, and code in Parallelism’s post chrome.
taxonomy:
...
Parallelism’s landing chrome (masonry columns) is louder than post bodies. This Elements leaf lives under `/blog` so CTAs stay real — it checks that inner articles still get a readable type scale.
