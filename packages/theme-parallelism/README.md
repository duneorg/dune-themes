...
Horizontal masonry portfolio with scroll zones. Adapted from
[HTML5 UP Parallelism](https://html5up.net/parallelism) (CC BY 3.0).

![Parallelism screenshot](https://themes.getdune.org/parallelism/themes/parallelism/static/screenshot.png)

**Demo**: https://themes.getdune.org/parallelism

**Tags**: dune-theme, portfolio, gallery, html5up
...
themes:
  - name: parallelism
    src: jsr:@dune/theme-parallelism@1.0.0
...
Horizontal masonry on home and blog, wheel/keyboard scroll, and edge
scroll zones. See the [live demo](https://themes.getdune.org/parallelism).

## Templates

| Template | Role |
|---|---|
| `default` | Masonry gallery landing on home; prose pages elsewhere |
| `blog` | Masonry thumb post listing |
| `post` | Single post |
...
- **No dark mode / no color-scheme presets** — upstream Parallelism is a single design; a Dune dark or multi-scheme lift may land later as an optional enhancement, not as fidelity work.
- **CC BY credit** remains visible when `show_html5up_credit` is on (compare [https://html5up.net/parallelism](https://html5up.net/parallelism)).
