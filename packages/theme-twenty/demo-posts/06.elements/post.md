...
template: post
published: true
summary: Headings, lists, quotes, and code in Twenty’s post chrome.
taxonomy:
...
Twenty’s landing chrome (twenty banner) is louder than post bodies. This Elements leaf lives under `/blog` so CTAs stay real — it checks that inner articles still get a readable type scale.
