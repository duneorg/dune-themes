    ...
    <html lang={page?.language ?? "en"} dir={dir ?? "ltr"} class={defaultDark ? "dark" : ""}>
      <head>
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){var s=localStorage.getItem('theme-halcyonic');
      ...
      <body class="theme-halcyonic archetype-landing">
        <header class="site-header">
          <a class="site-logo" href="/">{site?.title ?? "Halcyonic"}</a>
        ...
        <footer class="site-footer">
          {footerText || (
            <>
              © {new Date().getFullYear()} {site?.title ?? "Halcyonic"} ·{" "}
              {(themeConfig?.show_html5up_credit !== false) && (
                <>
                  <a href="https://html5up.net/halcyonic">Halcyonic by HTML5 UP</a> ·{" "}
                </>
              )}
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){
            ...
            btn.addEventListener('click',function(){
              ...
              localStorage.setItem('theme-halcyonic',dark?'dark':'light');
            });
          })();
        ` }} />
      </body>
