...
template: post
published: true
summary: Theme options for the Phantom demo — still a single upstream design.
taxonomy:
  tag: [config, phantom]
---
...
| Option | Notes |
|--------|--------|
| `logo_url` | see `theme.yaml` |
| `show_html5up_credit` | see `theme.yaml` |
| `footer_text` | see `theme.yaml` |

**No dark mode / no color-scheme presets.** Upstream Phantom is one design;
