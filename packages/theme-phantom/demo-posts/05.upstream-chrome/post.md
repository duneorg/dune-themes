...
template: post
published: true
summary: Theme-specific fidelity notes for Phantom — not a generic clone.
taxonomy:
  tag: [chrome, phantom, fidelity]
---

Upstream Phantom is known for **minimal tiles**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Minimal tile/grid portfolio-blog hybrid.

Compare side-by-side with [html5up.net/phantom](https://html5up.net/phantom):
