  ...
  return (
    <LayoutComponent {...props}>
      ...
      {items.map((post) => {
        ...
        return (
          <section key={post.route}>
            ...
            {excerpt && <p>{excerpt}</p>}
            <ul class="actions">
              <li><a href={post.route} class="button small">{tr("post.read_more", "Read")}</a></li>
            </ul>
            <hr />
          </section>
        );
      ...
      {(pagination?.newer || pagination?.older) && (
        <ul class="actions">
          {pagination.older && (
            <li><a href={pagination.older} class="button small">← {tr("pagination.older", "Older")}</a></li>
          )}
          {pagination.newer && (
            <li><a href={pagination.newer} class="button small">{tr("pagination.newer", "Newer")} →</a></li>
          )}
        </ul>
      )}
    </LayoutComponent>
