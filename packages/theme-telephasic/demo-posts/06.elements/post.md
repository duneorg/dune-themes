...
template: post
published: true
summary: Headings, lists, quotes, and code in Telephasic’s post chrome.
taxonomy:
...
Telephasic’s landing chrome (telephasic splits) is louder than post bodies. This Elements leaf lives under `/blog` so CTAs stay real — it checks that inner articles still get a readable type scale.
