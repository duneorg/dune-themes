    ...
    <html lang={page?.language ?? "en"} dir={dir ?? "ltr"} class={defaultDark ? "dark" : ""}>
      <head>
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){var s=localStorage.getItem('theme-strongly-typed');
      ...
      <body class="theme-strongly-typed archetype-blog-minimal">
        <header class="site-header">
          <a class="site-logo" href="/">{site?.title ?? "Strongly Typed"}</a>
        ...
        <footer class="site-footer">
          {footerText || (
            <>
              © {new Date().getFullYear()} {site?.title ?? "Strongly Typed"} ·{" "}
              {(themeConfig?.show_html5up_credit !== false) && (
                <>
                  <a href="https://html5up.net/strongly-typed">Strongly Typed by HTML5 UP</a> ·{" "}
                </>
              )}
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){
            ...
            btn.addEventListener('click',function(){
              ...
              localStorage.setItem('theme-strongly-typed',dark?'dark':'light');
            });
          })();
        ` }} />
      </body>
