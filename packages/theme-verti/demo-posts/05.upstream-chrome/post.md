...
template: post
published: true
summary: Theme-specific fidelity notes for Verti — not a generic clone.
taxonomy:
  tag: [chrome, verti, fidelity]
---

Upstream Verti is known for **verti sidebar**. On Dune that chrome is template-driven over vendored `static/html5up/` assets — this post exists so the demo explains what you’re seeing instead of shipping a generic clone.

Vertical nav energy with content column.
