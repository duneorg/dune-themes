---
title: PaperMod's typography
date: 2026-02-01
template: post
published: true
summary: How PaperMod sets headings, lists, blockquotes, code, and links.
taxonomy:
  tag: [typography, papermod]
---

PaperMod’s stylesheet already paints the usual markdown surfaces —
this post dogfoods them so you can see headings, lists, code, and tables
in the same chrome as a normal entry.
...
One size down again, for a subsection within a section. PaperMod doesn’t
...
| Option | Type | Default |
| --- | --- | --- |
| `ShowShareButtons` | toggle | `true` |
| `ShowReadingTime` | toggle | `true` |
| `defaultTheme` | select | `auto` |

## Links

**Bold**, *italic*, and [links](https://getdune.org) all read at the same
weight as the rest of the paragraph — nothing about PaperMod's typography
shouts.

---

That horizontal rule is the divider PaperMod’s stylesheet defines —
a plain line in the muted border colour used elsewhere in the chrome.
